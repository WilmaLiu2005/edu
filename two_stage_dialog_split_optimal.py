...
INPUT_FOLDER = "/Users/vince/undergraduate/KEG/edu/Data/full_student"   # 输入 CSV 文件夹
OUTPUT_FOLDER = "/Users/vince/undergraduate/KEG/edu/full_split" # 输出 CSV 文件夹
...
# Stage 1: 时间划分
# ==============================
def entry_and_time_split(df, time_threshold=15):
    results = []
    if df.empty:
        return results
    ...
    for i in range(1, len(df)):
        ...
        # 判断是否需要切分
        different_entry = curr_row["提问入口"] != prev_row["提问入口"]
        delta = (curr_row["提问时间"] - prev_row["提问时间"]).total_seconds() / 60
        too_far = delta > time_threshold

        if different_entry or too_far:
            # 保存当前块
            results.append(pd.DataFrame(current_chunk))
            current_chunk = [curr_row]
        else:
            current_chunk.append(curr_row)
...
def process_csv(file_path, output_folder):
    try:
        ...
        # Stage 1: 时间间隔划分
        time_splits = entry_and_time_split(df)  # 每块顺序保持不变
